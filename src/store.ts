	...
	actions: {
		fetchStats: async ({ commit }) => {
			...
			const [fetchedDownloads, fetchedStars, fetchedContributors] = await Promise.all([
				fetch('https://api.npmjs.org/downloads/range/2013-08-21:2100-08-21/up-devs.db').then(toJSON, noop),
				fetch('https://api.github.com/repos/Up-Devs/up-devs.db').then(toJSON, noop),
				fetch('https://api.github.com/repos/Up-Devs/up-devs.db/stats/contributors').then(toJSON, noop),
			]);
		...
		fetchDocs: async (
			...
			for (const x of documentation.externals) {
				documentation.links[x.name] = x.see[0].replace(/\{@link\s+(.+?)\s*\}/i, '$1').replace('Constants', '$1')
			}
			for (const c of documentation.classes) {
